...
export async function POST(request:Request){
    ...
    try{
        const isWatchlisted = await prisma.watchlistSerial.findFirst({
            where:{
        ...
        if(isWatchlisted){
            const removeWatchlist = await prisma.watchlistSerial.deleteMany({
            where:{
                userId:currentUser.user.id,
                serialId:id
            }}
        ...
        return NextResponse.json({message:"Sucesfully removed wachlist",watchlisted:false},{status:200})
            ...
            const addWatchlist = await prisma.watchlistSerial.create({
                data:{
                userId:currentUser.user.id,
                serialId:id
            }
