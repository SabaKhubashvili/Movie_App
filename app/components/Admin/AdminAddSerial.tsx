...
export const AdminAddSerial = ({tags}:Props) => {
    ...
    const onSubmit =  async(data: FieldValues) => {      
      if (!isLoading) {
        ...
        axios.post('/api/Serial/addSerial', {
          ...data,
        }, {
        }).then(async(res)=>{
          const params = {
    ...
    <section className='w-full pt-[150px] flex flex-col gap-[30px]'>
        ...
        <div className='flex gap-x-[30px] gap-y-[20px] md:flex-nowrap flex-wrap items-center'>
            <div className=' flex gap-[20px]'>
                <ImageUpload label='Big banner' onChange={(image)=>setCustomValue('serialBannerBig',image)} value={imageBig}/>
                <ImageUpload label='small banner' onChange={(image)=>setCustomValue('serialBannerSmall',image)} value={imageSmall}/>
            </div>
